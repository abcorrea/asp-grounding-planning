...
for config in CONFIGS:
    for counter in ['LPCNT_OPT_BIN_PATH', 'LPCNT_BIN_PATH', 'LPCNT_1M_BIN_PATH', 'LPCNT_500K_BIN_PATH']:
        for task in suites.build_suite(BENCHMARKS_DIR, SUITE):
            ...
            run.set_property('algorithm', '-'.join([config.name, counter]))
            run.set_property('id', ['-'.join([config.name, counter]), task.domain, task.problem])
